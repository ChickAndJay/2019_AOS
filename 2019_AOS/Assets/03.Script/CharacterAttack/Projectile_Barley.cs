...
public class Projectile_Barley : Projectile{
    ...
    public void TheStart(int damage, bool isSkill, Vector3 landingPosition )
    {
        ...
        //_length = (landingPosition -transform.position).magnitude;       //  not use rigidbody
        _length = (landingPosition - (transform.position + new Vector3(0,-1,0))).magnitude;  // use rigidbody

        //_bullet_up_speed = 9.8f * (_length / PlayerController.instance._playerStats._bulletRange); //  not use rigidbody
    ...
    void Update() {
        ...
        //// commentary is for not use gravity version
        //float t = (Time.time - _startTime);

        //float forwardCovered = t * _forward_velovity;
        //float forwardJourney = forwardCovered / _length;

        //if (t >= 2.0)
        //    t = 2f;
        //_yPos = 1+ (_bullet_up_speed * t - _gravity * t * t / 2);

        //transform.position = Vector3.Lerp(_startPosition , _landingPosition, forwardJourney);
    ...
    protected override IEnumerator DestroySelf(float time)
    {
        ...
        GameObject plane = GameObject.Instantiate(_attackPlane,
            initPos, 
            transform.rotation);
        plane.GetComponent<BarleyPlaneAttack>().TheStart(_damage);
